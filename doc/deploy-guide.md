...
> **コメント・リアクション機能について**: `docker compose up`（または `npm run dev:pages`）で起動すると、コメントやバナナのリアクションがローカルでも保存されます。データは `.wrangler/state/` に保持されるため、コンテナを再起動してもリロードで消えません。
...
> ローカル開発中（`npm run dev:pages`）はカウンターは動作せず、「—」と表示されます。これは正常です。コメント・リアクションは `.wrangler/state/` にローカル保存されます（本番KVとは別のデータです）。
